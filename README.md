...
├── tools/
│   ├── video/
│   │   ├── film.html              旧・動画の元（いまサイトに載っている動画はこれではない）
...
- 動画は1本だけ。プロフィールの冒頭に置いている（`assets/video/delivery-os.mp4`）。
  **このファイルは本人が別のところで作ったもの**で、このリポジトリの
  `tools/video/` からは生成していない。1080×1920 の H.264/AAC、1分53秒、
  **字幕は映像に焼き込み済み**なので `<track>` は付けない
- 差し替えるときは mp4 を置き換え、表紙（`delivery-os-poster.jpg`）を作り直し、
  `index.html` の分秒の表記と `?v=` を直す
- `tools/video/` は以前このリポジトリで動画を作っていたときの道具。
  いま載っている動画とは関係ない
